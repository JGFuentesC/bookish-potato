...
- **Próximo incremento**: E0-H2 — orquestación de contenedores (compose, imágenes, configuración)
...
3. E0-H1-T3 — inicializar módulos (Go, uv en data-platform y ai-sidecar, frontend Vite) ✅ (verificación: `go build`, `uv.lock` en ambos, `pnpm build`)
4. E0-H1-T4 — Makefile raíz (`bootstrap`, `verify`, `lint`, `test`, `fmt`, ...) ✅ (verificación: `make -n <meta>` en todas + `make verify` exit 0)
5. E0-H1-T5 — ADR-001 con versiones exactas del stack ✅ (verificación: ninguna celda con "≥" ni "por definir")
